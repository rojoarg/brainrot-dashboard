...
const PAGES_PER_CHUNK = 20;        // 20 pages per chunk ≈ 10s — safe even when Eldorado API is slow
...
const TOTAL_CALLS = Math.ceil(TOTAL_CHUNKS / CHUNKS_PER_CALL); // 53 batch calls → 55 hops total
...
function parseListing(item: any): Listing | null {
  ...
  const price = usdPrice.amount || 0;
  if (!name || price <= 0) return null;
  return {
    name, rarity: tev['Rarity'] || '', mutation: oav['Mutations'] || 'None',
    ms: oav['M/s'] || 'N/A', exact_ms: exactMs, price, quantity: o.quantity || 0,
    seller: u.username || '', verified: u.isVerifiedSeller || false, offer_id: o.id || '',
    seller_rating: uoi.feedbackScore || 0, seller_feedback_count: uoi.ratingCount || 0,
    seller_positive: uoi.positiveCount || 0, seller_negative: uoi.negativeCount || 0,
    seller_dispute_ratio: uoi.disputedAccountOrderRatio || 0,
...
function sanitizeText(val: string | null): string {
  if (!val) return '';
...
function toDbRow(l: Listing) {
  return {
    ...
    offer_title: sanitizeText(l.offer_title),
    delivery_time: sanitizeText(l.delivery_time),
    is_trending: l.is_trending,
    expire_date: l.expire_date, offer_state: l.offer_state,
    description: sanitizeText(l.description),
...
export async function GET(request: Request) {
  ...
  if (segment.startsWith('batch-')) {
    const batchNum = parseInt(segment.split('-')[1]);
...
// ─── INIT: Clear STAGING table (NOT live), create run ───
...
// ─── CHUNK: Scrape pages into STAGING table ───
async function scrapeSegment(chunkNum: number) {
  ...
  const deduped = allListings.filter(l => {
    if (!l.offer_id || seenIds.has(l.offer_id)) return false;
  ...
  if (runningRun) {
    ...
    const { count: realStagingCount } = await supabase
  ...
  return { chunk: chunkNum, pages: `${startPage}-${endPage}`, fetched: deduped.length, failed, upsertErrors, durationMs: Date.now() - t0 };
}

// ─── FINALIZE: Wait for all batches, then atomic swap staging→live ───
async function finalizeScrape() {
  ...
  if (runningRun) {
    ...
    let waited = 0;
    while (waited < 30000) { // max 30s wait
      const { data: check } = await supabase
        .from('brainrot_scrape_runs')
        .select('segments_completed')
        .eq('id', runningRun.id)
        .single();
      if (check && (check.segments_completed || 0) >= target) break;
  ...
  try {
    ...
    const snapshots = Object.entries(combos).map(([, items]) => {
      ...
      return {
        ...
        avg_price: Math.round((prices.reduce((s: number, p: number) => s + p, 0) / n) * 100) / 100,
        median_price: prices[Math.floor(n / 2)],
        listing_count: n,
