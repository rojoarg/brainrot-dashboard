    ...
    <div>
      ...
      <div className="glass-card table-wrap" style={{ maxHeight: '70vh' }}>
        <table className="dash-table" role="table">
          <thead><tr role="row">
            <th style={{ width: 28 }}></th><th>Name</th><th>Rarity</th><th>Mutation</th><th>M/s</th><th>Exact M/s</th><th>Price</th><th>Seller</th><th>Verified</th>
          </tr></thead>
          <tbody role="rowgroup">
            {filtered.length === 0 && (
            ...
            {filtered.map((t: TrendingItem) => (
              <tr key={`${t.name}-${t.offer_id}`} onClick={() => openDetail(t.name)} className="clickable" role="row">
                ...
                <td style={{ color: t.mutation !== 'None' ? 'var(--cyan)' : 'var(--text3)' }}>{t.mutation}</td>
                <td>{t.ms}</td>
                <td className="text-mono text-accent">{'-'}</td>
