  ...
  return (
    <div className="d-flex flex-col gap-6">
          ...
          <div className="glass-card p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={historyChart}>
                ...
                <YAxis tick={{ fontSize: 10, fill: 'var(--text3)' }} />
                <Tooltip contentStyle={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8 }} />
                <Area type="monotone" dataKey="avg" stroke="#ef4444" fill="url(#avgGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ) : (
      ...
      <div className="glass-card p-4">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={[...scatterData].sort((a: { name: string; score: number; med: number; tier: string; rarity: string; listings: number }, b: { name: string; score: number; med: number; tier: string; rarity: string; listings: number }) => b.score - a.score).slice(0, 30)}>
            ...
            <YAxis tick={{ fontSize: 10, fill: 'var(--text3)' }} />
            <Tooltip contentStyle={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8 }} formatter={(val) => [typeof val === 'number' ? val.toFixed(1) : String(val ?? '')]} />
            <Bar dataKey="score" fill="#ffd740" radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid-2col">
        <div className="glass-card p-4">
          <h3 className="d-flex items-center gap-2 mb-3" style={{ fontSize: 13, color: 'var(--gold)' }}>
            <TierBadge tier="S" /> Top S-Tier Brainrots
          </h3>
          {topS.map((r: Recommendation) => (
            <div key={r.name} onClick={() => openDetail(r.name)} className="config-item cursor-pointer mb-1">
              <ImageThumb src={r.imageUrl} />
              <div style={{ flex: 1 }}>
                <div className="text-lg fw-600">{r.name}</div>
                <div className="text-sm text-sub">
                  <RarityBadge rarity={r.rarity} /> {fmtPrice(r.med)} med &middot; {r.listings} listings
                  {r.soldCount > 0 && <span className="text-red" style={{ marginLeft: 4 }}>{r.soldCount} sold</span>}
                </div>
              </div>
              <div className="text-right">
                <div className="text-lg fw-700 text-gold">{r.score}</div>
                {r.roiPct > 0 && <div className="text-xs text-green">{r.roiPct}% ROI</div>}
              </div>
            </div>
          ))}
        </div>
        <div className="glass-card p-4">
          <h3 className="d-flex items-center gap-2 mb-3" style={{ fontSize: 13, color: 'var(--accent2)' }}>
          ...
          {topA.map((r: Recommendation) => (
            <div key={r.name} onClick={() => openDetail(r.name)} className="config-item cursor-pointer mb-1">
              ...
              <div className="text-right">
                <div className="text-lg fw-700" style={{ color: 'var(--accent2)' }}>{r.score}</div>
                {r.roiPct > 0 && <div className="text-xs text-green">{r.roiPct}% ROI</div>}
              </div>
            </div>
          ))}
      ...
      {soldArchive.recent.length > 0 && (
        <div className="glass-card p-4">
          <h3 className="text-lg text-red mb-3">Recently Sold / Delisted</h3>
          <div className="grid-strategies">
            {soldArchive.recent.slice(0, 12).map((s: SoldItem, i: number) => (
              <div key={i} onClick={() => openDetail(s.name)} className="config-item cursor-pointer">
      ...
      {/* Scrape history */}
      <div className="glass-card p-4">
        <h3 className="text-lg text-sub mb-3">Scrape History</h3>
            ...
            <tbody role="rowgroup">
              {(meta.scrapeRuns || []).map((r: { status: string; totalListings?: number; completed_at?: string; started_at?: string; pages_scraped?: number }, i: number) => (
                <tr key={`scrape-${r.completed_at}-${i}`} role="row">
                  ...
                  <td><span style={{ color: r.status === 'completed' ? 'var(--green)' : r.status === 'running' ? 'var(--gold)' : 'var(--red)' }}>{r.status}</span></td>
                  <td>{r.totalListings?.toLocaleString() || '-'}</td>
                  <td>{'-'}</td>
                  <td>{'-'}</td>
                  <td>{r.pages_scraped || '-'}</td>
                  <td style={{ color: false ? 'var(--red)' : 'var(--text2)' }}>{'0'}</td>
                  <td>{'-'}</td>
                  <td>{'-'}</td>
                  <td>{'-'}</td>
                  <td>{'-'}</td>
