          ...
          <tbody role="rowgroup">
            ...
            {sellers.slice(0, 200).map((s: Seller) => (
              <tr key={`seller-${s.name}`} role="row">
                ...
                <td style={{ color: s.rating >= 99 ? 'var(--green)' : s.rating >= 95 ? 'var(--gold)' : 'var(--text)' }}>{s.rating}%</td>
                ...
                <td style={{ color: s.disputeRatio > 1 ? 'var(--red)' : 'var(--text2)' }}>{s.disputeRatio}%</td>
                <td>{s.verified ? <span className="text-green">✓</span> : '-'}</td>
                <td>{s.warranty ? <span style={{ color: 'var(--cyan)' }}>✓</span> : '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
