...
export default React.memo(function DetailTab({ data, selected, setSelected, isOnWL, addToWL, removeFromWL }: DetailTabProps) {
  ...
  const bq = parseInt(buyQty) || 1;

  return (
    <div className="d-flex flex-col gap-4">
      {/* Nav bar */}
      <FilterBar>
        <button type="button" className="btn text-md" onClick={goPrev} disabled={currentIdx <= 0} style={{ padding: '4px 10px' }} aria-label="Previous item">{'\u2190'} Prev</button>
        <select className="select" value={selected} onChange={e => setSelected(e.target.value)} style={{ minWidth: 160 }}>
          {names.map(n => <option key={n} value={n}>{n}</option>)}
        </select>
        <button type="button" className="btn text-md" onClick={goNext} disabled={currentIdx >= recNames.length - 1} style={{ padding: '4px 10px' }} aria-label="Next item">Next {'\u2192'}</button>
        ...
        <div className="d-flex gap-4 flex-wrap items-center">
          ...
          {rec && (
            <div className="text-right">
              <div className="text-display fw-700" style={{ fontSize: 28, color: 'var(--gold)' }}>{rec.score}</div>
      ...
      <div className="glass-card p-3">
        ...
        <div className="d-flex gap-3 items-center flex-wrap">
          <div>
            ...
            <input className="input" type="number" value={buyPrice} onChange={e => setBuyPrice(e.target.value)} placeholder={b.minPrice.toFixed(2)} style={{ width: 120 }} />
          </div>
          <div>
            <label className="text-sm text-muted">Quantity</label>
            <input className="input" type="number" value={buyQty} onChange={e => setBuyQty(e.target.value)} style={{ width: 80 }} />
          </div>
          {bp > 0 && (
            <div className="d-flex gap-4" style={{ fontSize: 13 }}>
              ...
              <div><span style={{ color: b.medianPrice > bp ? 'var(--green)' : 'var(--red)' }}>
                Profit: {fmtPrice((b.medianPrice - bp) * bq)} ({((b.medianPrice - bp) / bp * 100).toFixed(0)}% ROI)
              </span></div>
              <div><span className="text-muted">Sell at P75:</span> <strong className="text-green">{fmtPrice((b.p75 - bp) * bq)} ({((b.p75 - bp) / bp * 100).toFixed(0)}%)</strong></div>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="grid-stats gap-3">
            {[
              ...
              <div key={s.label}>
                ...
                <div className="fw-700" style={{ fontSize: 18, color: s.color }}>{s.val}/{s.max}</div>
      ...
      {rec && (() => {
        ...
        return (
          <div className="glass-card p-3" style={{ borderColor: 'rgba(255,165,0,0.3)', background: 'linear-gradient(135deg, rgba(255,165,0,0.04), rgba(255,100,0,0.02))' }}>
            ...
            <div className="grid-strategies">
              {overrides.map(a => (
                <div key={a.mutation} className="d-flex items-center gap-2" style={{ padding: '8px 12px', borderRadius: 8, background: 'var(--bg)', border: '1px solid rgba(255,165,0,0.2)' }}>
                  <span className="fw-700 text-md" style={{ color: MUTATION_COLORS[a.mutation] || 'var(--text)', minWidth: 70 }}>{a.mutation}</span>
                  <div style={{ flex: 1, fontSize: 11 }}>
                    ...
                    <div className="text-muted" style={{ fontSize: 10 }}>{a.priceRatio}x base price · {a.listings} listings</div>
                  </div>
                  <div className="text-right">
                    <div className="fw-700 text-mono text-orange" style={{ fontSize: 14 }}>{fmtMinValue(a.recommendedOverride)}</div>
              ...
              <tr key={comboKey} role="row" style={isValuable ? { background: 'rgba(255,165,0,0.04)' } : undefined}>
                ...
                <td className="text-sm">
                  {c.sellers?.slice(0, 3).map((s: ComboSeller, j: number) => (
                    <span key={`${s.name}-${s.price}`} style={{ marginRight: 6 }}>
                      {s.verified && <span className="text-green">✓</span>}
                      {s.name} ({fmtPrice(s.price)})
                      {s.rating && s.rating > 0 && <span className="text-muted" style={{ marginLeft: 2 }}>{Math.round(s.rating)}%</span>}
                    </span>
                  ))}
                </td>
              </tr>
