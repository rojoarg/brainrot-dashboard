...
export default React.memo(function SoldTab({ data, openDetail }: SoldTabProps) {
  ...
  const filteredDelisted = useMemo(() => {
    if (!search) return marketChanges.delisted;
    const q = search.toLowerCase();
    return marketChanges.delisted.filter((c: MarketChange) => c.name.toLowerCase().includes(q));
  }, [marketChanges, search]);

  const filteredNew = useMemo(() => {
    if (!search) return marketChanges.newItems;
    const q = search.toLowerCase();
    return marketChanges.newItems.filter((c: MarketChange) => c.name.toLowerCase().includes(q));
  }, [marketChanges, search]);
      ...
      {view === 'sold' && (
        <div>
          ...
          <div className="glass-card p-4 mb-4">
            <h3 className="text-lg text-sub mb-3">Most Sold Brainrots (30d)</h3>
          ...
          {/* Recent sold table */}
          <div className="glass-card table-wrap" style={{ maxHeight: '50vh' }}>

            <table className="dash-table" role="table">
              <thead><tr role="row">
                <th style={{ width: 28 }}></th><th>Name</th><th>Rarity</th><th>Mutation</th><th>M/s</th><th>Price</th><th>Qty</th><th>Seller</th><th>Sold</th>
              </tr></thead>
              <tbody role="rowgroup">
                {filteredRecent.slice(0, 300).map((s: SoldItem, i: number) => (
                  <tr key={`${s.offer_id}-${i}`} onClick={() => openDetail(s.name)} className="clickable" role="row">
                    ...
                    <td style={{ color: s.mutation !== 'None' ? 'var(--cyan)' : 'var(--text3)' }}>{s.mutation}</td>
                    <td>{s.ms}</td>
            ...
            <tbody role="rowgroup">
              {(view === 'delisted' ? filteredDelisted : filteredNew).slice(0, 500).map((c: MarketChange, i: number) => (
                <tr key={`${c.name}-${c.detected_at}-${i}`} onClick={() => openDetail(c.name)} className="clickable" role="row">
                  ...
                  <td style={{ color: c.mutation && c.mutation !== 'None' ? 'var(--cyan)' : 'var(--text3)' }}>{c.mutation}</td>
                  <td>{c.ms}</td>
