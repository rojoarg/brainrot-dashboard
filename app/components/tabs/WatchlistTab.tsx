...
export default React.memo(function WatchlistTab({ data, config, openDetail, removeFromWL }: WatchlistTabProps) {
  ...
  return (
    <div>
      ...
      <div className="glass-card table-wrap" style={{ maxHeight: '75vh' }}>
              ...
              return (
                <React.Fragment key={item.pet_name}>
                  <tr onClick={() => openDetail(item.brainrotName)} className="clickable">
                    ...
                    <td>
                      {overrides.length > 0 ? (
                        <button type="button" className="btn text-orange text-xs" style={{ borderColor: 'rgba(255,165,0,0.3)', background: 'rgba(255,165,0,0.08)', padding: '2px 8px' }} onClick={(e) => { e.stopPropagation(); setExpandedItem(isExpanded ? null : item.pet_name); }} aria-label={`${overrides.length} mutation override${overrides.length > 1 ? 's' : ''} for ${item.brainrotName}`}>
                    ...
                    <td><button type="button" className="btn btn-danger text-sm" onClick={(e) => { e.stopPropagation(); removeFromWL(item.pet_name); }} style={{ padding: '2px 8px' }} aria-label={`Remove ${item.brainrotName} from watchlist`}>{'\u2715'}</button></td>
                  </tr>
                  {isExpanded && overrides.length > 0 && (
                    <tr>
                      <td colSpan={14} style={{ padding: '8px 12px', background: 'rgba(255,165,0,0.04)', borderTop: 'none' }}>
                        ...
                        <div className="d-flex flex-wrap gap-2">
                          {overrides.map((a: MutationAdvisory) => (
                            <div key={a.mutation} className="d-flex items-center gap-2 p-2" style={{ borderRadius: 6, background: 'var(--bg)', border: '1px solid rgba(255,165,0,0.2)', fontSize: 11 }}>
