...
export async function GET() {
  ...
  for (const [, b] of Object.entries(brainrots)) {
    ...
    b.medianPrice = n > 0 ? b.prices[Math.min(Math.floor(n / 2), n - 1)] : 0;
    b.p10 = n > 0 ? b.prices[Math.min(Math.floor(n * 0.1), n - 1)] : 0;
    b.p25 = n > 0 ? b.prices[Math.min(Math.floor(n * 0.25), n - 1)] : 0;
    b.p75 = n > 0 ? b.prices[Math.min(Math.floor(n * 0.75), n - 1)] : 0;
    b.p90 = n > 0 ? b.prices[Math.min(Math.floor(n * 0.9), n - 1)] : 0;
    ...
    for (const c of Object.values(b.combos) as any[]) {
      ...
      c.prices.sort((a: number, cn: number) => a - cn);
      const cn = c.prices.length;
      c.avgPrice = cn > 0 ? Math.round((c.totalPrice / cn) * 100) / 100 : 0;
      c.medianPrice = cn > 0 ? c.prices[Math.min(Math.floor(cn / 2), cn - 1)] : 0;
  ...
  const topSellers = Object.entries(sellers)
    ...
    .map(([name, s]) => ({
      ...
      positive: s.positive,
      negative: s.negative,
      disputeRatio: Math.round(s.disputeRatio * 10000) / 100,
      warranty: s.warranty,
      joined: s.joined,
  ...
  const sanitizeValue = (v: any): any => {
    ...
    if (v instanceof Set) return Array.from(v).map(sanitizeValue);
    if (Array.isArray(v)) return v.map(sanitizeValue);
    if (typeof v === 'object') {
      return Object.fromEntries(
        Object.entries(v).map(([k, val]) => [k, sanitizeValue(val)])
      );
    }
    return v;
  };
...
function buildRecommendations(brainrots: Record<string, any>, wlMap: Record<string, any>, blSet: Set<string>, soldByName: Record<string, any>) {
  ...
  for (const [name, b] of Object.entries(brainrots)) {
    ...
    const spread = b.minPrice > 0 ? ((b.maxPrice - b.minPrice) / b.minPrice) : 0;
    const roiAtMedian = b.minPrice > 0 ? ((medPrice - b.minPrice) / b.minPrice) : 0;
  ...
  for (const r of raw) {
    ...
    const demandFromSold = sold.count > 0 ? Math.min(6, Math.log2(sold.count + 1) * 2) : 0;
    ...
    const score = baseMarketScore + rarityBonus;
