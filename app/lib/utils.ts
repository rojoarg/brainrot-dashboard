...
export const fmt = (n: number) => {
  ...
  return n >= 1e6 ? (n / 1e6).toFixed(1) + 'M' : n >= 1e3 ? (n / 1e3).toFixed(1) + 'K' : n.toFixed(n < 10 ? 2 : 0);
};
 ...
 */
export function smartMinValue(rec?: any): number {
  ...
  const aggression = RARITY_AGGRESSION[rarity] ?? 0.3;
  ...
  const liquidityFactor = (listings ?? 0) >= 10 ? 1.0 : (listings ?? 0) >= 3 ? 0.9 : 0.75;
